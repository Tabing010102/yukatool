...
namespace Yuka.Utils
{
    public class SjisTunnelEncoding : Encoding
    {
        ...
        public static readonly Encoding StrSjisEncoding = Encoding.GetEncoding(932, EncoderFallback.ExceptionFallback, DecoderFallback.ReplacementFallback);
        ...
        public override string GetString(byte[] bytes, int index, int count)
        {
            ...
            while (i < index + count)
            {
                ...
                if ((highByte >= 0x81 && highByte < 0xA0) || (highByte >= 0xE0 && highByte < 0xFD))
                {
                    ...
                    {
                        StrSjisEncoding.GetChars(bytes, i - 2, 2, decodedChars, 0);
                        result.Append(decodedChars[0]);
                    }
                }
                else
                {
                    StrSjisEncoding.GetChars(bytes, i - 1, 1, decodedChars, 0);
                    result.Append(decodedChars[0]);
                }
            }
